...
#define DISPATCH_INPUT_TYPES(TYPE, TYPE_NAME, SCOPE_NAME, ...) \
  [&] { \
    ...
    { \
      ...
      PRIVATE_CASE_TYPE_AND_VAL(at::ScalarType::Half, at::Half, TYPE_NAME, 1024, __VA_ARGS__) \
      PRIVATE_CASE_TYPE_AND_VAL(at::ScalarType::BFloat16, at::BFloat16, TYPE_NAME, 1024, __VA_ARGS__) \
      default: \
...
namespace kaolin
{
  ...
  template <typename T>
  struct ScalarTypeToVec3
  {
    using type = void;
  };
  template <>
  struct ScalarTypeToVec3<float>
  {
    using type = float3;
  };
  template <>
  struct ScalarTypeToVec3<double>
  {
    using type = double3;
  };
  template <>
  struct ScalarTypeToVec3<at::Half>
  {
    using type = float3;
  };
  template <>
  struct ScalarTypeToVec3<at::BFloat16>
  {
    using type = float3;
  };

  template <typename T>
  struct Vec3TypeToScalar
  {
    using type = void;
  };
  template <>
  struct Vec3TypeToScalar<float3>
  {
    using type = float;
  };
  template <>
  struct Vec3TypeToScalar<double3>
  {
    using type = double;
  };
